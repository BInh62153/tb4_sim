...
ARGUMENTS = [
    ...
    # PERF: ogre2 (default) chạy nặng hơn nhiều so với ogre (Ogre1) khi render bằng
    # CPU/Xvfb (software rasterizer) vì ogre2 dùng pipeline PBR/shadow tốn hơn.
    # Trên đường GPU thật (USE_VIRTUALGL=true + EGL hoạt động) nên để ogre2.
    # Trên đường CPU/Xvfb fallback, đổi sang "ogre" giảm đáng kể CPU cho render thread.
    DeclareLaunchArgument('render_engine', default_value='ogre2',
                          choices=['ogre', 'ogre2'],
                          description='Gazebo render engine: ogre2 (GPU, đẹp hơn) hoặc ogre (Ogre1, nhẹ hơn cho CPU render)'),
]
    ...
    ignition_gazebo = IncludeLaunchDescription(
        ...
        launch_arguments=[
            ...
            ('gz_args', PythonExpression([
                '"', LaunchConfiguration('world'), '.sdf -r -v 2 --render-engine ',
